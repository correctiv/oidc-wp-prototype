...
echo; echo "━━━ Website: cookie hand-over and clear (HAProxy) ━━━"
BOGUS="abc.def.ghi"
check "12. GET /auth/session is refused"               "400" "$(status "$SITE/auth/session")"
check "    POST without the community Origin"          "403" "$(status -X POST -d "token=$BOGUS&return=/" "$SITE/auth/session")"
check "    POST with the Origin but a bogus token"     "400" "$(status -X POST -H "Origin: $COMMUNITY" -d "token=$BOGUS&return=/" "$SITE/auth/session")"
check "    POST with a self-signed token"              "400" "$(status -X POST -H "Origin: $COMMUNITY" -d "token=$(node -e "import('jose').then(async j=>{const k=new TextEncoder().encode('x'.repeat(32));console.log(await new j.SignJWT({example_role:'full',iss:'http://auth.example.localhost:8080/realms/example',aud:'community-app'}).setProtectedHeader({alg:'HS256'}).setExpirationTime('1h').sign(k))})" 2>/dev/null)&return=/" "$SITE/auth/session")"
check "13. /auth/clear removes the cookie"             "example_session=; *Max-Age=0*" "$(header set-cookie "$SITE/auth/clear?return=/members/")"
check "    ... and continues to the return path"       "/members/" "$(header location "$SITE/auth/clear?return=/members/")"
check "    ... but never to a foreign host"            "/" "$(header location "$SITE/auth/clear?return=https://evil.example/")"
check "    ... nor to a protocol-relative URL"         "/" "$(header location "$SITE/auth/clear?return=//evil.example/")"
check "    ... and is never cached"                    "no-store" "$(header cache-control "$SITE/auth/clear?return=/")"
check "14. /contact/me without login is 401"          "401" "$(status -H "Origin: $SITE" "$COMMUNITY/contact/me")"
